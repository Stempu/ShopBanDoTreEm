...
if (isPost()){
    ...
    if(!empty(trim($body['email'])) && !empty(trim($body['password']))){
        ...
        $userQuery = firstRaw("SElECT id,password,activeToken FROM users WHERE email='$email' AND status=1");

        if(!empty($userQuery)){
            $passwordHash = $userQuery['password'];
            $userID = $userQuery['id'];

            if($userQuery['activeToken'] != null){
                if(password_verify($password,$passwordHash)){
                    ...
                    if($insertStatus){
                        ...
                        redirect('admin?module=dashboard&action=list');
                    }else{
            ...
            setFlashData('msg', 'Email của bạn đang bị khóa vui lòng liên hệ Admin');
            setFlashData('msgType','danger');
        }
    }else{
...
<div class="card">
    <div class="card-body">
        ...
        <!-- /Logo -->
        <h4 class="mb-2">Chào mừng đến với  Sneat! 👋</h4>
        <?php
            getMsg($msg,$msgType);
        ?>
        ...
        <form id="formAuthentication" class="mb-3" action="" method="POST">
            ...
            <div class="mb-3 form-password-toggle">
                <div class="d-flex justify-content-between">
                    ...
                    <a href="<?php echo getLinkAdmin('auth','forgot') ?>"
